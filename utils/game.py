...
class Hangman:
    ...
    def start_game(self):
        """ method that: will call play() until the game is over (because the user guessed the word or because of a game over). will call game_over() if lives is equal to 0.
        will call well_played() if all the letter are guessed. will print correctly_guessed_letters, bad_guessed_letters, life, error_count and turn_count at the end of each turn. """
        while self.lives > 0:
            print(f"Correct letters:{self.correctly_guessed_letters} " + f"Bad letters: {self.wrongly_guessed_letters} " + f"Lives:{self.lives} " + f"Errors:{self.error_count} " + f"Turns:{self.turn_count}")
